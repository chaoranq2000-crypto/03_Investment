...
def _workflow_stage_path(project: str) -> Path:
    from investment_system.pipelines.sector_research.load_project import PROJECTS_ROOT
...
def _run_scope_check(project: str) -> list[StepResult]:
    return [
        _mark_load_warning_only(_run_module("investment_system.pipelines.sector_research.load_project", "--project", project, "--json")),
        _run_module("investment_system.pipelines.sector_research.audit_pipeline_readiness", "--project", project),
        _run_module("investment_system.pipelines.validate_outputs", "--project", project),
    ]


def _run_evidence_gate(project: str, sector_id: str) -> list[StepResult]:
    results = [
        _run_module("investment_system.pipelines.sector_research.audit_evidence_bindings", "--project", project),
        _run_module("investment_system.pipelines.sector_research.audit_evidence_schema", "--project", project),
        _mark_coverage_warning_only(
            _run_module("investment_system.pipelines.sector_research.audit_evidence_coverage", "--project", project),
            sector_id,
        ),
    ]
    return results
...
def _run_evidence_register(
    ...
    if not apply_registration:
        args.append("--dry-run")
    return [_run_module("investment_system.pipelines.sector_research.register_evidence_file", *args)]
...
def _run_evidence_collect(args: argparse.Namespace) -> list[StepResult]:
    ...
    return [_run_module("investment_system.pipelines.sector_research.collect_official_evidence", *command_args)]
...
def _run_evidence_draft(args: argparse.Namespace) -> list[StepResult]:
    ...
    return [_run_module("investment_system.pipelines.sector_research.build_evidence_skeleton", *command_args)]
...
def _run_generate_candidate(project: str, sector_id: str, run_id: str | None) -> list[StepResult]:
    ...
    return [_run_module("investment_system.pipelines.sector_research.build_formal_candidate_outputs", *args)]


def _run_candidate_gate(project: str, sector_id: str) -> list[StepResult]:
    return [_run_module("investment_system.pipelines.sector_research.audit_formal_candidate_outputs", "--project", project, "--sector-id", sector_id)]


def _run_publish_gate(project: str, sector_id: str, publish_scope: str) -> list[StepResult]:
    prepare = _run_module(
        "investment_system.pipelines.sector_research.prepare_formal_publish",
        "--project",
        project,
        "--sector-id",
        sector_id,
    ...
    readiness = _run_module("investment_system.pipelines.sector_research.audit_formal_publish_readiness", *args)
...
def _run_post_publish_check(project: str, sector_id: str) -> list[StepResult]:
    return [
        _run_module("investment_system.pipelines.sector_research.audit_formal_publish_result", "--project", project, "--sector-id", sector_id),
        _run_module("investment_system.pipelines.validate_outputs", "--project", project),
        _run_module("investment_system.pipelines.sector_research.audit_pipeline_readiness", "--project", project),
    ]
