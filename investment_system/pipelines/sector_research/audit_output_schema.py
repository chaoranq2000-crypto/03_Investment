...
def audit_project(project_id: str, write_report: bool = True) -> tuple[list[Finding], dict[str, Any]]:
    ...
    validate_path = WORKSPACE_ROOT / "investment_system" / "pipelines" / "validate_outputs.py"
    validate_content = validate_path.read_text(encoding="utf-8", errors="replace")
    validate_status = "ok" if "Output contract loaded" in validate_content and "validate_csv_contract" in validate_content else "missing"
