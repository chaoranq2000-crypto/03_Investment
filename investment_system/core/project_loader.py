"""
Core Project Loader for Sector Research Workspace.
...
project-aware skill scripts.

Usage (CLI):
    python -m investment_system.core.project_loader --project tech_ai_semiconductor
    python -m investment_system.core.project_loader --project tech_ai_semiconductor --dry-run-paths
    python -m investment_system.core.project_loader --project tech_ai_semiconductor --json
    python -m investment_system.core.project_loader --project tech_ai_semiconductor --create-output-dirs

Usage (library):
    from investment_system.core.project_loader import load_project, get_sector, list_scoring_sectors, get_stocks_for_sector, resolve_evidence_files_for_sector, resolve_output_paths
...
class ProjectConfig:
    ...
    def to_dict(self) -> dict[str, Any]:
        d = {
            ...
            "retired_output_count": self.raw.get("retired_output_count", 0),
...
def resolve_sector_card_path(
    ...
    When called with a sector_id string, it must be a canonical sector_id.
...
def resolve_output_paths(
    ...
    If sector_id is provided, also returns sector_card_path and raw_data_dir.
    ...
    """
    return _core_resolve_output_paths(config, sector_id)
...
def get_sector(config: ProjectConfig, sector_id: str) -> dict[str, Any]:
    """Look up a sector by canonical sector_id."""
    return _core_get_sector(config, sector_id)
...
def get_stocks_for_sector(
    ...
    *,
    ...
    Args:
        ...
        sector_id: canonical sector identifier
    ...
    Behavior:
      - sector_id must already be canonical.
...
def resolve_evidence_files_for_sector(
    ...
    Project-aware evidence binding is keyed by canonical sector_id:
      1. Validate the input as a canonical sector_id.
    ...
    This resolver intentionally ignores seed_documents and retired_outputs.
    """
    return _core_resolve_evidence_files_for_sector(config, sector_id)
...
def check_alias_key_conflicts(
    sectors: list[dict],
) -> list[ValidationWarning]:
    """
    Ensure alias metadata does not point to more than one canonical sector_id.
    Conflicts are errors because they indicate unsafe migration data.
    """
    warnings = []
    ...
    for key, canonicals in key_to_canonical.items():
        ...
        if len(unique) > 1:
            warnings.append(ValidationWarning(
                code="ALIAS_KEY_AMBIGUOUS",
                message=f"Alias key '{key}' maps to multiple canonical sector_ids: "
...
def check_manifest_planned_sector_refs(
    ...
    """run_manifest.planned_sectors[].sector_id must be canonical."""
    warnings = []
    for p in planned:
        sid = p.get("sector_id", "")
        if not sid:
...
def check_unregistered_md_seed_documents(
    ...
    seed_docs: list[dict],
    existing_outputs: list[dict],
    retired_outputs: list[dict] | None = None,
) -> list[ValidationWarning]:
    """
    ...
    for out in existing_outputs:
        ...
        if p:
            ...
            registered_paths.add(p)
    # Also exclude retired output paths from the unregistered-MD scan.
    if retired_outputs:
        for out in retired_outputs:
            p = out.get("path", "")
            if p:
                abs_p = WORKSPACE_ROOT / p
                registered_paths.add(str(abs_p.resolve()))
    ...
    for scan_root in scan_roots:
        ...
        for root, dirs, files in os.walk(scan_root):
            ...
            dirs[:] = [
                d
                for d in dirs
                if d not in MD_SCAN_EXCLUDE_DIRS and not d.startswith("_archive")
            ]
...
def load_project(
    ...
    *,
    ...
    retired_outputs = get_nested(manifest_yaml, "retired_outputs", default=[])
    ...
    all_warnings.extend(check_alias_key_conflicts(sectors))
    ...
    all_warnings.extend(check_manifest_planned_sector_refs(planned, valid_sector_ids))

    ev_warns, ev_file_warns = check_evidence_file_sector_refs(
        evidence_files, valid_sector_ids
    )
    ...
    all_warnings.extend(check_evidence_file_mapping_consistency(evidence_files, sectors))

    unregistered_warns = check_unregistered_md_seed_documents(
        project_dir, seed_docs, existing_outputs, retired_outputs
    )
    ...
    config = ProjectConfig(
        ...
        raw={
            ...
            "retired_output_count": len(retired_outputs),
...
def _print_dry_run_output_contract(config: ProjectConfig) -> None:
    ...
    for output_type in output_types:
        ...
        print(f"      optional_fields({len(optional)}): {optional}")
        print(f"      path: {path}")
    print()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  python -m investment_system.core.project_loader "
