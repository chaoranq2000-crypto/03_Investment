"""Small CLI delegation helpers for migrated skill wrappers."""
...
def run_module_main(module_name: str, argv: Sequence[str] | None = None) -> int:
    """Run a legacy module's main function with an argv-compatible surface."""
    args = list(argv or [])
    old_argv = sys.argv[:]
...
def print_command_help(skill_name: str, commands: Mapping[str, LegacyCommand]) -> None:
...
def dispatch_legacy_commands(
    skill_name: str,
    commands: Mapping[str, LegacyCommand],
        ...
        if "-h" not in delegated_args and "--help" not in delegated_args:
            if command.requires_flag not in delegated_args:
                print(
                    ...
                    f"Pass {command.requires_flag} to delegate to the legacy command.",
                    file=sys.stderr,
                )
                return 0
